...
export default function Contact() {
  ...
  return (
    <section id="contact" className="relative min-h-screen py-24 bg-[#020617] text-white overflow-hidden font-['Inter',sans-serif]">
      ...
      <motion.div className="absolute top-20 left-10 w-72 h-72 rounded-full blur-3xl bg-blue-500/10" animate={{ y: [0, 20, 0] }} transition={{ duration: 6, repeat: Infinity }} />
      <motion.div className="absolute bottom-20 right-10 w-96 h-96 rounded-full blur-3xl bg-blue-500/10" animate={{ y: [0, -20, 0] }} transition={{ duration: 7, repeat: Infinity }} />

      <div className="max-w-6xl mx-auto px-6 relative z-10 flex flex-col lg:flex-row gap-16 items-center">
        {/* LEFT SIDE */}
        <motion.div initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="lg:w-1/3 space-y-10 text-center lg:text-left">
        ...
        >
          ...
          >
            <AnimatePresence mode="wait">
              {status === "idle" ? (
                <motion.div key="idle" className="flex items-center gap-2">Send <Send size={14} /></motion.div>
              ) : (
